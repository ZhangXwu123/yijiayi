...
class StudentController extends CommonController{
    ...
    public function index(){
            ...
            $str ='';
            $url1 = $this->a_url("","YiJiaYicms","Student","is_switch",'',array("id","if"));
            $url2 = $this->a_url("","YiJiaYicms","Student","addup","",array("id","if"));
            $url3 = $this->a_url("","YiJiaYicms","Student","del","",array("id","if"));
           ...
           $this->a_url("add_url", "YiJiaYicms", "Student", "addup",'','',1);
           $title ="产品管理";
           $this->assign(array("ctitle"=>$title,'keyworld'=> $keyworld,"str"=>$str));
           $this->veiws("Student","index"); 
    }
        ...
        if($action && ($action == "add" || $action == "edit")){
                    ...
                    if($action == "edit"){
                                ...
                                if($result == "false"){
                                        ...
                                        $this->ShowMsgC("操作成功！", $this->a_url("", "YiJiaYicms", "Student", "index",'',array("id")));
                                }
                    }else{
         ...
         if ($upload == 1 && !empty($_FILES)) { //上传
                    $result = $this->uplodes($_FILES, date("YmdHis",thistime).$this->getRandChar(2),0, 0, 0, 'YiJiaYi/Student');
                    if ($result[0] > 0) {
            ...
            if(empty($id)){
                        ...
                        if($this->GetOne("select id from `".PR."product` where md5(md5(id))='".md5($id)."' ")){
                                    ...
                                    if(!(string)$result){
                                            ...
                                            $this->ShowMsgC("",$this->a_url("","YiJiaYicms","Student","index",'',array("id","if")));
                                    }
                        }
            }
    }
            ...
            if(empty($id)){
                ...
                if($this->GetOne("select id from `".PR."product` where md5(md5(id))='".md5($id)."' ")){
                            ...
                            if(!(string)$result){
                                    ...
                                    $this->ShowMsgC("删除成功！",$this->a_url("","YiJiaYicms","Student","index","","",1));
                            }
                }
            }
    }
